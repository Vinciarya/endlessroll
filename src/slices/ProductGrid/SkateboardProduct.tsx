...
async function getDomminantColor(url: string){
    const palatte = new URL(url);
    palatte.searchParams.set("palette","json");
    
    const res = await fetch(palatte);
...
const VERTICAL_LINE_CLASS = "absolute top-0 h-full text-stone-300 transition-colors group-hover:text-stone-400";
...
export async function SkateboardProduct({ id }: Props) {
  ...
  const price = isFilled.number(product.data.price)
    ...
    ? await getDomminantColor(product.data.image.url)
    :undefined;
    ...
    <div className="group relative mx-auto w-full max-w-72 px-8 pt-4 ">
      ...
      <div className="~mb-1 overflow-hidden py-4">
        ...
        <PrismicNextImage
          ...
          width={150}
          className="mx-auto w-[58%] origin-top transform-gpu transition-transform opacity-500 ease-in-out group-hover:scale-150"
        />
      </div>
      <HorizontalLine className={HORIZONTAL_LINE_CLASS}/>
